...
// Créer un repas
export const createReservationMeal = async (req, res) => {
  try {
...
export const getAllReservationMeal = async (req, res) => {
  try {
...
export const getReservationMealById = async (req, res) => {
  try {
...
export const updateReservationMeal = async (req, res) => {
  try {
    const { date, description, allergenes } = req.body;
    const meal = await Meal.findByPk(req.params.id);
    if (!meal) return res.status(404).json({ message: "Repas introuvable" });
...
// Supprimer un repas
export const deleteReservationMeal = async (req, res) => {
  try {
    const meal = await Meal.findByPk(req.params.id);
    if (!meal) return res.status(404).json({ message: "Repas introuvable" });
